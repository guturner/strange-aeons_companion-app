...
def _generate_inventory_header(armor=False, spells=False, weapons=False, instruments=False, misc_items=False):
...
def _generate_inventory_tuple(item, armor=False, spells=False, weapons=False, instruments=False, misc_items=False):
...
def _generate_inventory_tuples(items, armor=False, spells=False, weapons=False, instruments=False, misc_items=False):
    return tuple(map(lambda item: _generate_inventory_tuple(item, armor=armor, spells=spells, weapons=weapons, instruments=instruments, misc_items=misc_items), items))

def _get_inventory_data(should_lookup: bool, lookup_fn, armor=False, spells=False, weapons=False, instruments=False, misc_items=False):
    if not should_lookup:
        return ()

    result = lookup_fn()
    if isinstance(result, LookupInventorySuccess):
        return _generate_inventory_tuples(items=result.inventory, armor=armor, spells=spells, weapons=weapons, instruments=instruments, misc_items=misc_items)
    return ()
        ...
        match lookup_merchant_result:
            case LookupMerchantSuccess(merchant=found_merchant):
                ...
                armor_inventory_data = _get_inventory_data(
                    ...
                    armor=display_armor_columns, instruments=display_instrument_columns, spells=display_spell_columns, weapons=display_weapon_columns, misc_items=has_custom_misc_items
                )
                ...
                headers = _generate_inventory_header(armor=display_armor_columns, instruments=display_instrument_columns, spells=display_spell_columns, weapons=display_weapon_columns, misc_items=has_custom_misc_items)

                rows = general_goods_inventory_data + instruments_inventory_data + spells_inventory_data + weapons_inventory_data + armor_inventory_data + jewelry_inventory_data + inventory_data
